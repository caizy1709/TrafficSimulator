...
//#########################################
/*
roadID: just for identification of road
roadLen: road length
nLanes: #lanes

densInitPerLane: param
*/
...
function road(roadID, roadLen, nLanes, densInitPerLane, speedInit, truckFracInit, isRing) {
  ...
  for (var i = 0; i < this.nveh; i++) {
    ...
    var truckFracRest = (this.nLanes * truckFracInit > 1) ? ((this.nLanes * truckFracInit - 1) / (this.nLanes - 1)) : 0;
    var truckFrac = (lane == this.nLanes - 1) ? truckFracRight : truckFracRest;
    var r = Math.random();
    // var vehType = (Math.random() < truckFrac) ? "truck" : "car";
    var vehType = "car";
    var vehLength = car_length;
    var vehWidth = car_width;
...
road.prototype.writeVehicles = function() {
  ...
  for (var i = 0; i < this.veh.length; i++) {
    console.log(" veh[" + i + "].u=" + parseFloat(this.veh[i].u, 10).toFixed(1) + "  lane=" + this.veh[i].lane + "  speed=" + parseFloat(this.veh[i].speed, 10).toFixed(1) + "  acc=" + parseFloat(this.veh[i].acc, 10).toFixed(1) + "  iLead=" + this.veh[i].iLead + "  iLag=" + this.veh[i].iLag + "  iLeadRight=" + this.veh[i].iLeadRight + "  iLagRight=" + this.veh[i].iLagRight + "  iLeadLeft=" + this.veh[i].iLeadLeft + "  iLagLeft=" + this.veh[i].iLagLeft + "");
  }
}
...
road.prototype.writeVehiclesSimple = function() {
  console.log("\nin road.writeVehicles(): roadLen=" + this.roadLen);
  for (var i = 0; i < this.veh.length; i++) {
    console.log(" veh[" + i + "].u=" + parseFloat(this.veh[i].u, 10).toFixed(1) + "  lane=" + this.veh[i].lane + "  speed=" + parseFloat(this.veh[i].speed, 10).toFixed(1) + "  acc=" + parseFloat(this.veh[i].acc, 10).toFixed(1) + "");
  }
}
...
road.prototype.setOfframpInfo = function(offrampIDs, offrampLastExits, offrampToRight) {
...
road.prototype.getNextOffIndex = function(u) {
  ...
  for (var iOff = 0;
    (!success) && (iOff < this.offrampIDs.length); iOff++) {
...
road.prototype.setCFModelsInRange = function(umin, umax, longModelCar, longModelTruck) {
...
road.prototype.setLCModelsInRange = function(umin, umax, LCModelCar, LCModelTruck) {
...
road.prototype.setLCMandatory = function(umin, umax, toRight) {
  for (var i = 0; i < this.veh.length; i++) {
    ...
    if ((u > umin) && (u < umax)) {
      ...
      this.veh[i].LCModel = (toRight) ? this.LCModelMandatoryRight : this.LCModelMandatoryLeft;
    }
  }
}
...
road.prototype.calcAccelerations = function() {
  for (var i = 0; i < this.nveh; i++) {
    ...
    if (this.veh[i].isStop) {
      if (new Date().getTime() - this.veh[i].stopBegin > this.veh[i].stopPeriod) {
        this.veh[i].isStop = false;
        this.veh[i].acc = 6;
        this.veh[i].speed = 10;
        console.log(new Date().getTime());
        console.log("vehicle: " + i + this.veh[i]);
      } else {
        this.veh[i].acc = 0;
      }
    } else {
      this.veh[i].acc = this.veh[i].longModel.calcAcc(s, speed, speedLead, accLead);
    }
    //if(false){
    //if(this.veh[i].mandatoryLCahead){
    //if(speed>1.05*this.veh[i].longModel.v0){
    if (false) {
      console.log("after calcAccelerations: i=" + i + " pos=" + this.veh[i].u + " lane=" + this.veh[i].v + " s=" + s + " speed=" + speed + " v0=" + this.veh[i].longModel.v0 + " speedLead=" + speedLead + " acc=" + this.veh[i].acc
...
road.prototype.updateSpeedPositions = function() {
  for (var i = 0; i < this.nveh; i++) {
    ...
    if (this.veh[i].speed < 0) {
      this.veh[i].speed = 0;
    }
    if (this.veh[i].u > 400  && this.veh[i].u < 410 && !this.veh[i].isStop  && !this.veh[i].hasStopped) {
      var speed = this.veh[i].speed;
      this.veh[i].isStop = true;
      this.veh[i].stopBegin = new Date().getTime();
      this.veh[i].speed = 0;
      this.veh[i].stopPeriod = 1000;
      this.veh[i].hasStopped = true;
    }
...
road.prototype.changeLanes = function() {
  ...
  for (var i = 0; i < this.nveh; i++) {
    ...
    console.log("veh: " + i + " choose to " + newLane);
    if (newLane == 'stay') {
      continue;
    } else if (newLane =='left') {
      this.doChangesInDirection(0);
    } else {
      this.doChangesInDirection(1);
    }
...
road.prototype.mergeDiverge = function(newRoad, offset, uStart, uEnd, isMerge, toRight) {
    ...
    var success = ((targetVehicles.length == 0) && (originVehicles.length > 0) && (originVehicles[0].type != "obstacle") && (originVehicles[0].mandatoryLCahead));
    if (success) {
      iMerge = 0;
    ...
    // else select the first suitable candidate on the origin lane (if any)
      ...
      for (var i = 0;
        (i < originVehicles.length) && (!success); i++) { // merging veh loop
        if ((originVehicles[i].type != "obstacle") && (originVehicles[i].mandatoryLCahead)) {
          ...
          for (var j = 0; j < targetVehicles.length; j++) {
            ...
            if (log) {
              console.log("  du=" + du + " duLeader=" + duLeader + " duFollower=" + duFollower);
          ...
          var bBiasMerge = (toRight) ? 0.5 * bSafeMergeMax : -0.5 * bSafeMergeMax; // strong urge to change
          ...
          success = MOBILOK && (originVehicles[i].type != "obstacle") && (sNew > 0) && (sLagNew > 0) && (originVehicles[i].mandatoryLCahead);

          if (log && (this.roadID == 2)) {
            console.log("in road.mergeDiverge: roadID=" + this.roadID + " LCModel.bSafeMax=" + LCModel.bSafeMax);
          }
          if (success) {
            iMerge = i;
          }
          ...
          if (success && log) {
            console.log("testing origin veh " + i + " type=" + originVehicles[i].type + " uTarget=" + uTarget);
            ...
            console.log("  duLeader=" + duLeader + "  duFollower=" + duFollower + " sLagNew=" + sLagNew + " MOBILOK=" + MOBILOK + " success=" + success);
          }
        } // !obstacle
    ...
    if (success) { // do the actual merging
      ...
      if (log) {
        //if(true){
        console.log("Actual merging: merging origin vehicle " + iOrig + " of type " + this.veh[iOrig].type + " from origin position " + this.veh[iOrig].u + " and origin lane" + originLane + " to target position " + uTarget + " and target lane" + targetLane);
        console.log(" this.veh[iOrig].mandatoryLCahead)=" + this.veh[iOrig].mandatoryLCahead);
...
road.prototype.updateDensity = function(density) {
    ...
    if (this.nveh > nDesired) { // too many vehicles, remove one per time step
      ...
      if (success) { // otherwise, no veh added
        ...
        if (emptyLanes) {
          vehNew.speed = vehNew.longModel.v0;
        }
...
road.prototype.updateBCdown = function() {
  ...
  if ((!this.isRing) && (this.veh.length > 0)) {
    ...
    if (this.nveh < nvehOld) {
      this.updateEnvironment();
    }
  }
}

//a range function with good performance
//copied from stackoverflow
//can't believe there isn't a builtin in js
...
//######################################################################
road.prototype.updateBCup = function(Qin, dt, route, availableLanes) {
  ...
  if (this.inVehBuffer >= 1) {
    ...
    if (!success) {
      var spaceMax = 0;
      if (!availableLanes) {
        availableLanes = range(0, this.nLanes);
      }
      for (var candLanei = availableLanes.length; candLanei >= 0; candLanei--) {
        var candLane = availableLanes[candLanei];
        var iLead = this.nveh - 1;
    ...
    if (success) {
      ...
      this.nveh++;
      if (false) {
        console.log("road.updateBCup: new vehicle at pos u=0, lane " + lane + ", type " + vehType + ", s=" + space + ", speed=" + speedNew);
        console.log(this.veh.length); //!!!!
        for (var i = 0; i < this.veh.length; i++) {
          console.log("i=" + i + " this.veh[i].u=" + this.veh[i].u + " this.veh[i].route=" + this.veh[i].route);
        }
      }
...
road.prototype.getTargetNeighbourhood = function(umin, umax, targetLane) {
  ...
  if (false) {
    console.log("in road.getTargetNeighbourhood(umin=" + umin + ", umax=" + umax + ", targetLane=" + targetLane + ")");
...
road.prototype.updateModelsOfAllVehicles = function(longModelCar, longModelTruck,
  ...
  for (var i = 0; i < this.nveh; i++) {
    if (this.veh[i].type != "obstacle") { // then do nothing
      this.veh[i].longModel = (this.veh[i].type == "car") ? longModelCar : longModelTruck;
      this.veh[i].LCModel = (this.veh[i].type == "car") ? LCModelCar : LCModelTruck;
    }
  }
  ...
  if (this.duTactical > 0)
    for (var i = 0; i < this.nveh; i++) {
      ...
      if ((this.veh[i].type != "obstacle") && (iNextOff > -1) && (uLastExit - this.veh[i].u < this.duTactical)) {
        if (false) {
          console.log("in road.updateModels... iveh=" + i + " iNextOff=" + iNextOff + " u=" + this.veh[i].u + " uLastExit=" + uLastExit);
        }
        ...
        if (mandatoryLC) {
          ...
          this.veh[i].LCModel = (toRight) ? this.LCModelMandatoryRight : this.LCModelMandatoryLeft;
          if (false) {
            console.log("apply mandatoryLC to Vehicle " + i + "!" + "route=" + this.veh[i].route + " offID=" + offID + " uLastExit=" + uLastExit + " u=" + this.veh[i].u + " alpha_v0=" + this.veh[i].longModel.alpha_v0 + " bBiasRight=" + this.veh[i].LCModel.bBiasRight);
          }
...
road.prototype.draw = function(roadImg, scale, traj_x, traj_y, laneWidth,
    ...
    if (changedGeometry) {
      ...
      for (var iSegm = 0; iSegm < this.draw_nSegm; iSegm++) {
        ...
        if (false) {
          console.log("road.draw: iSegm=" + iSegm + " u=" + u + " xPhys=" + this.draw_x[iSegm] + " yPhys=" + this.draw_y[iSegm] + " phi=" + this.draw_phi[iSegm]);
        }
      }
    }
...
road.prototype.drawVehicles = function(carImg, truckImg, obstacleImg, scale,
  ...
  for (var i = 0; i < this.veh.length; i++) {
    if (noRestriction || ((this.veh[i].u >= umin) && (this.veh[i].u <= umax))) {
      ...
      var phiVehRel = (Math.abs(this.veh[i].dvdu) < 0.00001) ? 0 : -Math.atan(this.veh[i].dvdu);
      ...
      if (false) {
        //if(this.veh[i].v>2){
        console.log("in road.drawVehicles:" + " u=" + this.veh[i].u + " v=" + this.veh[i].v + " xCenterPix=" + xCenterPix + " yCenterPix=" + yCenterPix);
      }
    }
  }
}
